...
## Ajustes de seguridad para etiqueta y panel de tienda

- La etiqueta imprimible no muestra total, precios ni metodo de pago. Solo muestra datos logisticos para Shalom u otra agencia opcional.
- En `/admin`, un usuario de tienda ve solo las tiendas asociadas en `store_members`. Si solo tiene una tienda asignada, no aparece selector de tienda.
- `/platform` es el panel interno de Live Market para crear tiendas y administrar la plataforma. No se entrega a clientes finales.
- El total sigue apareciendo en el panel de pedidos y en el mensaje de WhatsApp, porque eso es para gestion interna y cierre de venta con el cliente.

## Actualización mobile-first, fotos, live y etiquetas

Esta versión ajusta Live Market para uso real desde celular:

- El panel de tienda usa navegación inferior en celular: Inicio, Productos, Live, Pedidos y Configuración.
- El producto en vivo ahora tiene botón **Quiero este producto** en la tienda pública. El cliente puede elegir variante, cantidad y agregarlo al pedido sin buscar manualmente el código.
- Productos permite subir imagen desde archivo/cámara del celular. En Supabase usa el bucket público `product-images`.
- El formulario de envío pide nombres y apellidos completos, DNI, celular, departamento, provincia, distrito, oficina Shalom destino y referencia.
- La agencia principal es Shalom. Se mantiene un campo opcional para otra agencia.
- La etiqueta de envío ya no muestra total, precios, método de pago ni contenido del paquete. Solo muestra datos logísticos para pegar en el paquete.
- `database.sql` agrega `customer_dni` a `orders`. Si el proyecto ya existía en Supabase, vuelve a ejecutar este SQL completo para aplicar la columna y reemplazar la función `create_public_order`.

### Importante para producción

Para usar el selector completo de Perú a nivel nacional, esta demo incluye una base local de departamentos con provincias y distritos comunes. Antes de escalar comercialmente, reemplazar o ampliar `src/lib/peru-locations.js` con un dataset oficial de UBIGEO completo.
