...
begin
  ...
  if coalesce(trim(p_customer->>'customer_name'), '') = ''
    or coalesce(trim(p_customer->>'customer_dni'), '') = ''
  ...
  insert into orders (
    store_id, order_code, customer_name, customer_dni, phone, social_user, city, department, province, district,
  ...
  values (
    ...
    nullif(trim(coalesce(p_customer->>'customer_dni', '')), ''),
