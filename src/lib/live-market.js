...
export async function getAdminStores() {
  ...
  if (supabase) {
    ...
    if (isPlatform) {
      ...
      if (error) throw error;
      return (data || []).map((store) => ({ ...store, store_settings: Array.isArray(store.store_settings) ? store.store_settings[0] : store.store_settings }));
    }
    const { data, error } = await supabase
      .from("store_members")
    ...
    return (data || []).map((row) => {
      ...
      return store ? { ...store, member_role: row.role, store_settings: Array.isArray(store.store_settings) ? store.store_settings[0] : store.store_settings } : null;
    }).filter(Boolean);
  }
  const db = readDemoDb();
  const memberships = db.store_members.filter((member) => member.user_id === session.user.id);
  return memberships.map((member) => hydrateStore(db, db.stores.find((store) => store.id === member.store_id))).filter(Boolean);
}
...
export function buildLabel(store, order) {
  const agency = order.shipping_agency === "Otra" && order.shipping_agency_other ? order.shipping_agency_other : order.shipping_agency || "Shalom";
  return [
    `TIENDA: ${store.name}`,
    ...
    "",
    "CONTENIDO:",
    ...(order.order_items || []).map((item) => `${item.quantity}x ${item.product_code} ${item.product_name}${item.variant_label ? ` (${item.variant_label})` : ""}`),
...
export async function assignUserToStore(storeId, userEmail, role = "store_staff") {
  if (supabase) {
    const { data, error } = await supabase.rpc("assign_user_to_store_by_email", {
      p_store_id: storeId,
      p_email: userEmail,
      p_role: role,
    });
    if (error) throw error;
    return { note: data || "Usuario asignado." };
  }
  const db = readDemoDb();
